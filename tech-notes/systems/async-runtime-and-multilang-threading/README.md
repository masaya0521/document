...
- Java の並行性モデルの変遷（1:1 スレッド → Virtual Threads）と JIT/AOT のトレードオフ
- バックエンド言語選択の判断基準
...
## 6. Java の並行性モデル — 1:1 スレッドから Virtual Threads へ

### 従来のモデル: 1:1 OS スレッド + Thread Pool

Java は長らく **1 つの Java スレッド = 1 つの OS スレッド** という 1:1 モデルを採用してきた。Node.js のシングルスレッド・非同期モデルとは対極にある。

```
Node.js（1スレッド、非同期 I/O）:
  Thread 1: [req1: DB開始] → [req2: DB開始] → [req3: DB開始]
                                              → [req1: DB結果] → [req2: DB結果]

従来の Java（マルチスレッド、同期 I/O）:
  Thread 1: [req1: DB開始 ─── 待ち ─── DB結果 → レスポンス]
  Thread 2: [req2: DB開始 ─── 待ち ─── DB結果 → レスポンス]
  Thread 3: [req3: DB開始 ─── 待ち ─── DB結果 → レスポンス]
```

Node.js は **1 人が複数の鍋を見ながら料理する**イメージ。Java は **料理人を何人も雇って、1 人 1 品を担当させる**イメージ。

| | Node.js | 従来の Java |
|---|---|---|
| スレッド数 | 1 | 多数（デフォルト 200 等） |
| I/O の待ち方 | 非同期（待たずに次へ） | 同期（待ってブロック） |
| 同時処理の仕組み | Event Loop で切り替え | 複数スレッドで並列 |
| 弱点 | CPU 処理で全体が詰まる | スレッド数が上限になると詰まる |

Spring Boot (Tomcat) のデフォルトはスレッドプール 200 本。I/O 待ちの間もスレッドが占有されるため、同時接続が 200 を超えるとリクエストがキューで待たされる。

### Java 21: Virtual Threads（Project Loom）

Java 21（2023 年 9 月）で **Virtual Threads** が正式導入された。Go の goroutine と非常に似た M:N モデル。

```
Java 21 以降（Virtual Threads）:
┌──────────────────────────────────────────┐
│            JVM プロセス                    │
│                                          │
│  Virtual Thread: vt1 vt2 vt3 ... vt10000 │  ← 数万〜数十万作れる
│                   ↓   ↓   ↓              │
│  JVM スケジューラが割り当てる               │
│                   ↓   ↓       ↓          │
│  Carrier Thread: [T1]      [T2]   [T3]   │  ← OS スレッド（コア数程度）
│                   ↓          ↓      ↓    │
│  CPU コア:     [Core1]   [Core2] [Core3]  │
└──────────────────────────────────────────┘
```

Virtual Thread が I/O 待ちになると、JVM が自動的にその Virtual Thread を Carrier Thread（実際の OS スレッド）から外し、別の Virtual Thread を載せる。Go の goroutine や tokio のタスクスケジューリングと本質的に同じ。

| | OS スレッド（従来の Java） | Virtual Thread | goroutine |
|---|---|---|---|
| メモリ | 1〜8 MB | 数百 B〜数 KB | 数 KB（初期 2KB） |
| 同時数 | 数百〜数千 | 数百万 | 数十万 |
| I/O 待ち時 | スレッドがブロック | 自動で Carrier Thread を解放 | 自動で OS スレッドを解放 |
| スケジューリング | OS | JVM（ForkJoinPool） | Go ランタイム（GMP） |

### k8s スケーリングでの起動の遅さ問題

Java の弱点は**起動の遅さ**。k8s のオートスケーリングで実際に問題になる。

```
トラフィック急増時:

  Go / Node.js の場合:
    Pod 起動 → 数十〜数百ms で Ready → すぐリクエスト処理開始

  Java (Spring Boot) の場合:
    Pod 起動 → JVM 起動 → クラスロード → DI コンテナ初期化 → Ready
    ├──────── 5〜30秒 ────────┤
                    この間リクエストが捌けない
```

起動に時間がかかる原因は **量 × 処理の重さ** の掛け算：

```
Java (Spring Boot) の起動シーケンス:

  1. JVM 起動                    ← 数百ms
  2. クラスロード（数千クラス）     ← 1〜3秒
  3. Spring DI コンテナ初期化      ← 2〜10秒（Bean のスキャン・生成）
  4. DB コネクションプール確立      ← 1〜3秒
  5. JIT コンパイル（本番速度到達） ← 数十秒〜数分

  合計 Ready まで: 5〜30秒
  本番性能に到達: 数分
```

Java は言語設計として「すべてがクラス」であり、フレームワーク文化として抽象化の層が厚い（Controller → Service → Repository → JPA → Hibernate → JDBC）。このため Go や Rust と比べてロード対象のクラス数が桁違いに多い。

| | Go (Gin) | Node.js (Express) | Java (Spring Boot) |
|---|---|---|---|
| 最小構成のクラス/モジュール数 | 数十 | 数十〜百 | **数千〜数万** |
| 起動時間 | 数十 ms | 数百 ms | **5〜30 秒** |

### JIT vs AOT — なぜ最初から全部コンパイルしないのか

Java の起動が遅い一因は JIT（Just-In-Time）コンパイル。最初はインタプリタで実行し、「よく使われるコード」を検知してから段階的にマシンコードにコンパイルする。

**最初から全部コンパイルしない理由は、実行時の情報を使った方がより良い最適化ができるから。**
...
void process(Animal a) {
    a.speak();  // Dog? Cat? コンパイル時にはわからない
}
```

- **AOT（事前コンパイル）**: `a` が Dog か Cat かわからないので、毎回型を確認する間接呼び出しが必要
- **JIT**: 実行中に「100 回中 100 回 Dog」と観測 → Dog.speak() への直接呼び出しに書き換え → さらに関数の中身をインライン展開 → 呼び出しオーバーヘッドがゼロに

さらに JIT は実行中の CPU を検知して、その CPU 固有の高速命令（AVX-512 等）を使える。AOT は「どの CPU で動くかわからない」ため、汎用的な命令セットでコンパイルせざるを得ない。

| | AOT（Go, Rust, GraalVM Native Image） | JIT（Java HotSpot, V8） |
|---|---|---|
| 起動速度 | 速い（コンパイル済み） | 遅い（インタプリタから開始） |
| ピーク性能 | 固定（ビルド時に決まる） | **AOT より高くなれる**（実行時最適化） |
| 最適化の質 | コンパイル時の情報だけ | 実行時プロファイルに基づく投機的最適化 |
| メモリ | 少ない | 多い（JIT コンパイラ自体がメモリを使う） |
| 向いている場面 | 短命プロセス、k8s スケーリング | 長時間稼働するサーバー |

### 起動の遅さへの対策

**1. GraalVM Native Image** — Java コードを事前にネイティブバイナリにコンパイル。起動が数十 ms に。ただしリフレクションに制限があり、ビルドが遅く、JIT がないためピーク性能は従来より低い場合がある。Quarkus や Micronaut がこれを前提に設計されている。

**2. CRaC (Coordinated Restore at Checkpoint)** — JVM のスナップショットをウォームアップ完了後に保存し、k8s ではそこから復元起動する。数百 ms で Ready かつウォームアップ済みの状態で起動可能。

**3. 最小 Pod 数を多めに確保** — スケールアウトに頼らず、常に余裕のある Pod 数を維持する。コスト効率は下がるが確実。
...
言語選択の判断フロー（バックエンド）:
  ...
  エンタープライズ / 既存 Java 資産がある？
    └─ Yes → Java（Spring Boot + Virtual Threads）が自然な選択

  チームが Rust/Go を書ける？
    ├─ No → TypeScript で十分なケースが多い
    └─ Yes → 要件次第
                ├─ I/O 中心の CRUD API → どれでも大差ない
                ├─ 高トラフィック + 共有状態 → Rust / Go / Java が有利
                └─ 極限のレイテンシ要件 → Rust が最適
```
...
| **Virtual Threads** | Java 21 で導入された軽量スレッド。goroutine と同様の M:N モデル |
| **JIT** | Just-In-Time コンパイル。実行時にホットスポットを検知してマシンコードに変換する |
| **AOT** | Ahead-Of-Time コンパイル。事前にマシンコードに変換する（Go, Rust, GraalVM Native Image） |
| **GraalVM Native Image** | Java コードを事前にネイティブバイナリにコンパイルする仕組み |
| **CRaC** | Coordinated Restore at Checkpoint。JVM のスナップショットから高速復元起動する仕組み |
| **HotSpot** | Oracle の JVM 実装。JIT コンパイラの名称でもある |
